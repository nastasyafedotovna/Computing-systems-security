...
def get_p(n: int, nod: list):
    global running
    x_i, x_j = random.randint(1, n - 2), 1
    i, j = 0, 2
    d = math.gcd(abs(x_i - x_j), n)
    while d == 1 and running:
        if i == j:
            x_j = x_i
            j *= 2
        x_i = (x_i ** 2 + 1) % n
...
def parallel(n: int):
    ...
    for i in range(count):
        th = Thread(target=get_p, args=(int(n), nod))
        threads.append(th)
        th.start()
...
#    temp = n // 8
#    d = []
#    starts = [temp * 2, temp * 4, temp * 6]
#    executor = concurrent.futures.ProcessPoolExecutor()
#    with executor as executor:
#        d.extend([i for i in executor.map(floyd, starts)])
#        print(d)
#    for e in d:
#        if 1 < e < n:
#            return d
